...
(function($) {
  ...
  FirechatUI.prototype = {
    ...
    _onChatInvite: function(invitation) {
      ...
      var $prompt = this.prompt('초대하기', template(invitation));
    ...
    $('#firechat-btn-rooms').bind('click', function() {
      ...
      self._chat.getRoomList(function(rooms) {
        ...
        for (var roomId in rooms) {
          ...
          rnamelist.push(rooms[roomId].name);
        ...
        var eatflag=true
        var conflag=true;
        if (rnamelist.length!=7)
        {
        	 for (var rname in rnamelist){
        	
        	if (rnamelist[rname]==(stname+" 일반")){
        		genflag=false;
        	}
        	if (rnamelist[rname]=="1:1 만나기"){
        		oneflag=false;
        	}
        	if (rnamelist[rname]=="N:N 만나기"){
        		nflag=false;
        	} 	
        	if (rnamelist[rname]=="구인 구직"){
        		emflag=false;
        	}
        	if (rnamelist[rname]=="사고 팔기"){
        		dealflag=false;
        	}
        	if (rnamelist[rname]=="카풀 찾기"){
        		carflag=false;
        	}
        	if (rnamelist[rname]=="핫플 공연"){
        		conflag=false;
        	}
        }
        if(genflag){
        	self._chat.createRoom((stname+" 일반"));
        }
        if(oneflag){
        	self._chat.createRoom("1:1 만나기");
        }
        if(nflag){
        	self._chat.createRoom("N:N 만나기");
        }
        if(emflag){
        	self._chat.createRoom("구인 구직");
        }
        if(dealflag){
        	self._chat.createRoom("사고 팔기");
        }
        if(carflag){
        	self._chat.createRoom("카풀 찾기");
        }
        if(conflag){
        	self._chat.createRoom("핫플 공연");
        }
        }
  ...
  FirechatUI.prototype.toggleInputs = function(isEnabled) {
	...
	function post_to_url(path, params, method) {
		var iframe =document.getElementById('tsop');
	    method = "post";  //method 부분은 입력안하면 자동으로 post가 된다.
	    //var iframe=document.createElement("iframe");
	    //iframe.setAttribute("style","display:none");
	    //iframe.setAttribute("frame_border","0");
	    var form = document.createElement("form");
	    form.setAttribute("method", method);
	    form.setAttribute("action", path);
	   	form.setAttribute("target", "tsop");
  ...
  FirechatUI.prototype.attachTab = function(roomId, roomName) {
    ...
    $textarea.bind('keydown', function(e) {
      ...
      if ((e.which === 13) && (message !== '')) {
        $textarea.val('');
        //bot start
        if (message.includes("공식"))
        {
        	message=("공식핫플봇 사칭은 허용되지 않습니다");
        }
        if (message.includes("밥"))
        {
        	message+="<<고갱님 "+stname+"에서는 맥모닝이 유명합니다. 해피밀하시죠>>";
        	post_to_url('https://hotple-psypu.c9users.io:8080/searchpost/'+stname+'/'+'밥', {'station':stname,'keyword':'밥','roomkey':room.id},"javascript:void(0)");
        	//보내는거 까지는 된다 하지만 realtime database sdk가 파이썬에 없어서 Django 에서 db push가 안됨
        }
        if (message.includes("술"))
        {
        	message+="<<고갱님 "+stname+"에서는 한신포차가 유명합니다. 약주한잔하시죠>>";
        	post_to_url('https://hotple-psypu.c9users.io:8080/searchpost/'+stname+'/'+'술', {'station':stname,'keyword':'술','roomkey':room.id},"javascript:void(0)");
        	//보내는거 까지는 된다 하지만 realtime database sdk가 파이썬에 없어서 Django 에서 db push가 안됨
        }
        self._chat.sendMessage(roomId, message);

        //if (message.includes("많"))
        //{
        //	post_to_url('https://hotple-psypu.c9users.io:8080/searchpost/', {'station':stname,'keyword':'많','roomkey':("-"+room.id)},"javascript:void(0)");
        //}
        //$.post("https://hotple-psypu.c9users.io:8080/searchpost/", {'station':stname,'keyword':'안녕'}); 
        $textarea.val('');
        return false;
      }
    });
  ...
  FirechatUI.prototype.formatTime = function(timestamp) {
    ...
    return date.getFullYear()+"년 "+(date.getMonth()+1)+"월 "+date.getDate()+"일 "+days[date.getDay()]+ ampm + hours + ':' + minutes+':' +seconds;
  };
  ...
  FirechatUI.prototype.promptCreateRoom = function() {
    ...
    var $prompt = this.prompt('방을만들어보세요', template({
